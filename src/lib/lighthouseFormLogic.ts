...
export function initLighthouseFormLogic(): void {
  ...
  (() => {
    ...
    // If page.tsx sets window.API_URL, we respect it. Otherwise default:
    const API_URL =
    ...
    const stepLineFill = document.getElementById("stepLineFill") as HTMLElement | null;
    ...
    const duplicateMsg = document.getElementById("duplicateMsg") as HTMLElement | null;
    ...
    // UPDATED: amount input controls (free-text with live commas)
    const amountInput = document.getElementById("amount") as HTMLInputElement | null;
    const amountRaw = document.getElementById("amountRaw") as HTMLInputElement | null;

    const needGroup = document.getElementById("needGroup") as HTMLElement | null;
    const needValue = document.getElementById("needValue") as HTMLInputElement | null;
    ...
    const submitError = document.getElementById("submitError") as HTMLElement | null;
    ...
    const clearDuplicateLock = document.getElementById("clearDuplicateLock") as HTMLButtonElement | null;
    const clearRetryQueue = document.getElementById("clearRetryQueue") as HTMLButtonElement | null;
    ...
    const parseNumber = (str: string) => Number((str || "").replace(/[^\d]/g, "")) || 0;
    ...
    /* ============================================================
       Amount live comma formatting (VISIBLE #amount + HIDDEN #amountRaw)
       ============================================================ */
    if (amountInput && amountRaw) {
      amountInput.addEventListener("input", (e: Event) => {
        ...
        el.value = formatted;
        amountRaw.value = String(parseNumber(formatted));

        let newPos = pos + diff;
        if (pos === before.length) newPos = formatted.length;
        try {
      ...
      // Ensure raw is correct on blur too
      amountInput.addEventListener("blur", () => {
        amountRaw.value = String(parseNumber(amountInput.value));
      });
    }

    /* ============================================================
       Mortgage need card select
       ============================================================ */
    ...
    /* ============================================================
       Errors
       ============================================================ */
    const invalidate = (id: string, message: string, inputEl?: HTMLElement | null) => {
      const el = err[id];
      if (!el) return;
    ...
    const clearAllErrors = () => {
      ...
      document.querySelectorAll(".error-ring").forEach((el) => el.classList.remove("error-ring"));
      document.querySelectorAll(".border-red-500").forEach((el) => el.classList.remove("border-red-500"));
    };

    const validateStep1 = () => {
      if (!form || !needGroup || !amountInput) return false;
      clearAllErrors();
      ...
      // Amount validation
      const amt = parseNumber(amountInput.value);
      if (!amt || amt < 1000) {
        invalidate("amount", "Enter an amount of at least $1,000.", amountInput);
        ok = false;
      }

      // Keep hidden raw in sync (even if user pasted weird formatting)
      if (amountRaw) amountRaw.value = String(amt);
    ...
    const validateStep2 = () => {
      ...
      const digits = ((phone as any)?.dataset?.digits as string) || (phone?.value.replace(/\D/g, "") || "");
    ...
    const writeDuplicateLock = (fingerprint: string) =>
      localStorage.setItem(duplicateKey, JSON.stringify({ fingerprint, ts: now() }));

    // Make fingerprint stable (do NOT include timestamp)
    function buildFingerprintPayload(payload: any) {
      const clone = { ...(payload || {}) };
    ...
    const writeQueue = (arr: Array<{ payload: any; err: string; ts: number }>) =>
    ...
    /* ============================================================
       Build payload (amount from input, numeric)
       ============================================================ */
    const buildPayload = () => {
      ...
      const phoneDigits =
        ((phoneInput as any).dataset?.digits as string) || phoneInput.value.replace(/\D/g, "");
   ...
   {/**  async function trySubmit(payload: any) {
      ...
      try {
        ...
        window.setTimeout(() => {
          const THANK_YOU_URL = "https://www.lighthouselending.ca/thank-you-page";
          // Use _top for iframe-safe navigation
...
/** Facebook pixel ends here */
  ...
  if (window.parent !== window) {
    window.parent.postMessage(
      ...
      "https://www.lighthouselending.ca"
    );
  }

  const payload = buildPayload();

  /** Google Tracking end here */

      const fingerprint = await sha256Hex(JSON.stringify(buildFingerprintPayload(payload)));
